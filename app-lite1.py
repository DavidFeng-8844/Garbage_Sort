# 此版本新功能
# 1.静止检测的初步实现。由于算法返回的物体按照置信度排序，所以要重新排序使得物体能够一一对应（即同一个物体在同一行，如物体A在50组数据中始终在第一行）
# 此版本中通过x坐标进行排序，但是当两个物体x坐标相近时容易出现问题（排序可能错误）
import cv2
import sys
import torch
...
class MainWindow(QWidget, Ui_Form):
    ...
    def detect(self):
        # 创建一个空间，存储最近五次识别的结果，并在循环结束时判断，最近五次的结果中，所有物体的左上角坐标是否在+-20的范围内，如果是就认为识别成功，并把每一个物体的结果取平均值作为最终结果
        last_five_results = []
        while self.run_flag:
            ...
            if success:
                ...
                # 把本次结果添加到last_five_results，覆盖最早的一次结果
                last_five_results.append(data)
                if len(last_five_results) > 40:
                    last_five_results.pop(0)
                    # 调用check_result_static函数，并返回是否静止的布尔值object_static和result
                    object_static, result = self.check_result_static(
                        last_five_results)
                    if object_static:
                        # 识别成功，把结果发给aurduino并等待返回
                        self.pushButton.click()
                        self.label_title.setText('识别结果：'+str(result))
                cv2.waitKey(1)
            else:
    ...
    def check_result_static(self, last_five_results):
        # 检查五次识别中，物体数量是否一致，如果不一致就返回False
        object_num = len(last_five_results[0])
        for result in last_five_results:
            if len(result) != object_num:
                return False, None
        # 对每一次识别的结果进行排序，按照物体的x坐标从小到大排序。如果x坐标相近，则按y坐标排序
        for result in last_five_results:
            result.sort(key=lambda x: (x[2][0], x[2][1]))
        # 检查五次识别中，所有物体的左上角坐标是否在+-20的范围内，如果是就认为识别成功
        for i in range(object_num):
            ...
            if max(x) - min(x) > 50 or max(y) - min(y) > 50:
                return False, None
        # 把每一个物体的结果取平均值作为最终结果
        result = []
        for i in range(object_num):
            x = []
            y = []
            for result0 in last_five_results:
                x.append(result0[i][2][0])
                y.append(result0[i][2][1])
            x = sum(x) / len(x)
            y = sum(y) / len(y)
            # result.append([i+1, result[i][1], (x, y), result[i][3], result[i][4], result[i][5]])
            result.append([result0[i][1], (x, y)])
        return True, result
